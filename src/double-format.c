...
validate_double_format (char const *fmt)
{
  ...
  for (i = 0; !(fmt[i] == '%' && fmt[i + 1] != '%'); i += (fmt[i] == '%') + 1)
    if (!fmt[i])
      die (EXIT_FAILURE, 0, _("format %s has no %% directive"), quote (fmt));
  ...
  if (!fmt[i])
    die (EXIT_FAILURE, 0, _("format %s missing valid type after '%%'"),
         quote (fmt));

  if (!strchr ("efgaEFGA", fmt[i]))
    die (EXIT_FAILURE, 0,
         _("format %s has unknown/invalid type %%%c directive"), quote (fmt),
         fmt[i]);
