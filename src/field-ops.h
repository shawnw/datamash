...
*/
enum FIELD_OP_COLLECT_RESULT field_op_collect (struct fieldop *op,
...
#define field_op_ok(X)                                                        \
  (((X) == FLOCR_OK) || ((X) == FLOCR_OK_KEEP_LINE)                           \
   || ((X) == FLOCR_OK_SKIPPED))
...
/* resets internal variables, should be called when starting a new
   group of values. */
void field_op_reset (struct fieldop *op);
...
void field_op_print_empty_value (enum field_operation mode);
