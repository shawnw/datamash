...
long double _GL_ATTRIBUTE_PURE
percentile_value (const long double *const values, const size_t n,
                  const double percentile)
{
  ...
  assert (n > 0 && percentile >= 0.0
          && percentile <= 100.0); /* LCOV_EXCL_LINE */
...
{
  if ((l == 4)
      && (STREQ_LEN (s, ".gpg", 4) || STREQ_LEN (s, ".bz2", 4)
          || STREQ_LEN (s, ".std", 4)))
    return true;
  if ((l == 3)
      && (STREQ_LEN (s, ".gz", 3) || STREQ_LEN (s, ".xz", 3)
          || STREQ_LEN (s, ".lz", 3)))
    return true;
  return false;
}
  ...
  if (l > 0 && s[l] == '.')
    {
      ...
      size_t ext_len
          = (prev_extension == 0) ? (len - l) : (prev_extension - l);
