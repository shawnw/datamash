  ...
  if (op->num_values >= op->alloc_values)
    {
      ...
      op->values
          = xnrealloc (op->values, op->alloc_values, sizeof (long double));
    }
  ...
  if (sort)
    {
      ...
      qsort (ptrs, op->count, sizeof (char *),
             sort_case_sensitive ? cmpstringp : cmpstringp_nocase);
    }
  return ptrs;
}
...
void
field_op_init (struct fieldop * /*out*/ op, enum field_operation oper,
               bool by_name, size_t num, const char *name)
{
  ...
  op->numeric
      = (op->acc_type == NUMERIC_SCALAR || op->acc_type == NUMERIC_VECTOR);
...
fields %" PRIuMAX ",%" PRIuMAX " have different number of items"),
         ...
         (uintmax_t)op->slave_op->field, (uintmax_t)op->field);
}
  ...
  if (op->numeric)
    {
      ...
      if (slen >= sizeof (tmpbuf))
        die (EXIT_FAILURE, 0, "internal error: input field too long (%zu)",
             slen);
  ...
  if (op->res_type == NUMERIC_RESULT)
    {
      ...
      snprintf (op->out_buf, op->out_buf_alloc, numeric_output_format,
                numeric_result);
    }
}
...
field_op_summarize (struct fieldop *op)
{
  ...
  switch (op->op) /* LCOV_EXCL_BR_LINE */
    ...
    case OP_PVARIANCE:
      numeric_result
          = variance_value (op->values, op->num_values, DF_POPULATION);
      break;

    case OP_SVARIANCE:
      numeric_result = variance_value (op->values, op->num_values, DF_SAMPLE);
      break;
    ...
    case OP_P_SKEWNESS:
      numeric_result
          = skewness_value (op->values, op->num_values, DF_POPULATION);
      break;

    case OP_S_EXCESS_KURTOSIS:
      numeric_result
          = excess_kurtosis_value (op->values, op->num_values, DF_SAMPLE);
      break;

    case OP_P_EXCESS_KURTOSIS:
      numeric_result
          = excess_kurtosis_value (op->values, op->num_values, DF_POPULATION);
      break;
    ...
    case OP_DP_OMNIBUS:
      numeric_result
          = dagostino_pearson_omnibus_pvalue (op->values, op->num_values);
      break;
    ...
    case OP_S_COVARIANCE:
      ...
      numeric_result = covariance_value (
          op->values, op->slave_op->values, op->num_values,
          (op->op == OP_P_COVARIANCE) ? DF_POPULATION : DF_SAMPLE);
      break;
    ...
    case OP_S_PEARSON_COR:
      ...
      assert (op->slave_op); /* LCOV_EXCL_LINE */
      verify_slave_num_values (op);
      numeric_result = pearson_corr_value (
          op->values, op->slave_op->values, op->num_values,
          (op->op == OP_P_PEARSON_COR) ? DF_POPULATION : DF_SAMPLE);
      break;
    ...
    case OP_BASE64:
      ...
      base64_encode (op->str_buf, op->str_buf_used - 1, op->out_buf,
                     BASE64_LENGTH (op->str_buf_used - 1) + 1);
      break;
