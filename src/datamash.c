  ...
  for (size_t i = 0; i < dm->num_grps; ++i)
    {
      ...
      if (p->num == 0)
        die (EXIT_FAILURE, 0, _("column name %s not found in input file"),
             quote (p->name));
      p->by_name = false;
    }
}
...
"),
      ...
      fputs ("\n\n", stdout);
      fputs (_("Print the sum and the mean of values from column 1:"),
             stdout);
      printf ("\n\
...
{
  error (0, 0,
         _("invalid input: field %" PRIuMAX " requested, "
      ...
      if (!field_op_ok (flocr))
        {
          ...
          die (EXIT_FAILURE, 0,
               ...
               field_op_collect_result_name (flocr), (uintmax_t)line_number,
               (uintmax_t)op->field, tmp);
        }
      ...
      if (crosstab_mode)
        {
          ...
          safe_line_record_get_fieldz (line, row_field, row_name,
                                       sizeof row_name);

          const size_t col_field = dm->grps[1].num;
          safe_line_record_get_fieldz (line, col_field, col_name,
                                       sizeof col_name);
  ...
  while (true)
    {
      if (num_lines + 1 > alloc_lines)
        {
          alloc_lines += 1000;
          lines
              = xnrealloc (lines, alloc_lines, sizeof (struct line_record_t));
        }
      ...
      if (strict && line_number > 1 && num_fields != prev_num_fields)
        die (EXIT_FAILURE, 0,
             _("transpose input error: line %" PRIuMAX " "
  ...
  while (true)
    {
      ...
      if (strict && line_number > 1 && num_fields != prev_num_fields)
        die (EXIT_FAILURE, 0,
             _("reverse-field input error: line "
...
tabular_check_file ()
{
  ...
  while (true)
    {
      ...
      /* Check if the number of fields changed from one line to the next
         (only if no expected number of fields specified on the command
         line).*/
  ...
  if (n_lines && n_lines != line_number)
    {
      die (EXIT_FAILURE, 0,
           _("check failed: input had %" PRIuMAX " lines "
              "(expecting %" PRIuMAX ")"),
  ...
  printf (ngettext ("%" PRIuMAX " line", "%" PRIuMAX " lines",
                    select_plural (line_number)),
          (uintmax_t)line_number);
  fputs (", ", stdout);
  printf (ngettext ("%" PRIuMAX " field", "%" PRIuMAX " fields",
                    select_plural (prev_num_fields)),
          (uintmax_t)prev_num_fields);
  print_line_separator ();
...
remove_dups_in_file ()
{
  ...
  thisline = &lr;
  line_record_init (thisline);
  ht = hash_initialize (init_table_size, NULL, hash_pjw, hash_compare_strings,
                        NULL);
      ...
      if (next_key_pos + len + 1 > keys_buffer_alloc)
        {
          ...
          if (buffer_list_size == buffer_list_alloc)
            buffer_list = x2nrealloc (buffer_list, &buffer_list_alloc,
                                      sizeof (char *));
      ...
      switch (optc)
        ...
        case UNDOC_PRINT_NAN_OPTION:
          field_op_print_empty_value (
              (optc == UNDOC_PRINT_INF_OPTION) ? OP_MAX : OP_MEAN);
          exit (EXIT_SUCCESS);
          break;
